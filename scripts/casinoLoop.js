...
async function getLoopState(userId, username) {
  ...
  if (userId) {
    ...
    me = {
      ...
      vipClaimed: stats.vip_claim_date === kstDateStr(),
  ...
  return {
    ...
    happyHour: happy,
    happyHourLabel: '매일 20:00–21:00 (KST) 승리 +10%',
    winners,
    me
  };
}
...
async function claimVipDaily(userId) {
  ...
  const today = kstDateStr();
  if (stats.vip_claim_date === today) {
    const err = new Error('오늘 VIP 보너스를 이미 받았습니다.');
    err.status = 400;
    throw err;
  }
  await pool.query('UPDATE user_casino SET vip_claim_date = ? WHERE user_id = ?', [today, userId]);
