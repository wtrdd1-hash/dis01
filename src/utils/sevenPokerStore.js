...
function playingError() {
  const err = new Error('이미 진행 중인 세븐포커가 있습니다.');
  err.status = 409;
  err.code = 'SP_IN_PROGRESS';
  return err;
}
...
async function openAndHoldBet(userId, source, betAmount, balanceBefore, state) {
  ...
  try {
    ...
    const [existing] = await conn.query(
      'SELECT user_id FROM seven_poker_sessions WHERE user_id = ? AND status = ? FOR UPDATE',
      [id, 'playing']
    );
    if (existing.length) {
      await conn.rollback();
    ...
    await conn.query(
      `INSERT INTO seven_poker_sessions (user_id, source, bet, balance_before, status, state_json)
       VALUES (?, ?, ?, ?, 'playing', ?)`,
...
async function increaseBet(userId, extraAmount) {
  ...
  try {
    ...
    const [rows] = await conn.query(
      'SELECT bet FROM seven_poker_sessions WHERE user_id = ? AND status = ? FOR UPDATE',
      [id, 'playing']
    );
    if (!rows.length) {
      await conn.rollback();
    ...
    if (!deduct.affectedRows) {
      await conn.rollback();
      const err = new Error('추가 배팅을 위한 잔액이 부족합니다.');
      err.code = 'INSUFFICIENT_CASH';
      err.status = 400;
      throw err;
    }
    await conn.query(
      'UPDATE seven_poker_sessions SET bet = bet + ? WHERE user_id = ? AND status = ?',
...
async function updateSession(userId, { bet, state } = {}) {
  ...
  params.push(id);
  await pool.query(
    `UPDATE seven_poker_sessions SET ${fields.join(', ')} WHERE user_id = ? AND status = 'playing'`,
    params
  );
}

async function getPlaying(userId) {
  const [rows] = await pool.query(
    `SELECT user_id, source, bet, balance_before, status, state_json
     FROM seven_poker_sessions WHERE user_id = ? AND status = ? LIMIT 1`,
...
async function claimSession(userId, nextStatus) {
  ...
  try {
    await conn.beginTransaction();
    const [rows] = await conn.query(
      'SELECT user_id, source, bet, balance_before, status FROM seven_poker_sessions WHERE user_id = ? AND status = ? FOR UPDATE',
      [id, 'playing']
    );
    if (!rows.length) {
      await conn.rollback();
    ...
    await conn.query(
      'DELETE FROM seven_poker_sessions WHERE user_id = ? AND status = ?',
      [id, 'playing']
    );
    await conn.commit();
    return {
...
async function refundOpenSessions() {
  const [rows] = await pool.query(
    'SELECT user_id FROM seven_poker_sessions WHERE status = ?',
    ['playing']
  );
  let count = 0;
  for (const row of rows) {
    try {
      ...
      console.error(`[seven-poker] 재시작 환불 실패 (${row.user_id}):`, err.message);
    }
  }
  try {
    await pool.query(
      `DELETE FROM seven_poker_sessions
...
async function hasOpenSession(userId) {
  const [rows] = await pool.query(
    'SELECT user_id FROM seven_poker_sessions WHERE user_id = ? AND status = ? LIMIT 1',
    [String(userId), 'playing']
  );
